...
ssize_t scull_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    bool flag = 0;
    int bytes_left, q_left_to_read, init_bytes_left;
    int read_more_q = 0;
    int nbytes;
    *f_pos = r_offset;
    ...
    {
        bytes_left = count - (quantum - q_pos);
        count = quantum - q_pos;
        read_more_q = 1;
    }
    printk(KERN_INFO "read: count %i\n", count);
    ...
    *f_pos += count;
    q_pos = r_offset % scull_quantum;
    // Если надо дочитать
    if (!read_more_q)
        goto skip;
    // Пробуем прочитать сверх 1 кванта
    s_pos = s_pos + 1;
    init_bytes_left = bytes_left;
    q_left_to_read = (bytes_left / scull_quantum) + (bytes_left % scull_quantum == 0 ? 0 : 1);
    printk(KERN_INFO "bytes_left: %i\n", bytes_left);
    printk(KERN_INFO "q_left_to_read: %i\n", q_left_to_read);
    for (int i = 0; i < q_left_to_read; i++) // && dptr->data[s_pos]
    {
        if (bytes_left > scull_quantum)
            nbytes = scull_quantum;
        else
            nbytes = bytes_left;
        if (copy_to_user(buf + scull_quantum * (i+1), dptr->data[s_pos+i], nbytes))
        {
            retval = -EFAULT;
            printk("Провал 1");
            goto out;
        ...
        if (!dptr->data[s_pos]) {
            printk("память не выделена");
        }
        printk(KERN_INFO "read: dptr->data[s_pos] %s ::: nbytes %i\n", dptr->data[s_pos+i], nbytes);
        if (bytes_left - scull_quantum > 0)
            bytes_left -= scull_quantum;
        else
            bytes_left = 0;
    }
    *f_pos += init_bytes_left;
    retval += init_bytes_left;
    printk("Доп. чтение выполено");
...
ssize_t scull_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    int bytes_left, quant_left, init_s_pos;
    /* Дополнение кода */
    ...
    *f_pos += nbytes; /* обновление позиции файла */
    ...
    bytes_left = count - nbytes;
    quant_left = bytes_left / scull_quantum + (bytes_left % scull_quantum == 0 ? 0 : 1);
    init_s_pos = s_pos + 1;
    q_pos = 0;
    init_bytes_left = bytes_left;
    for (int i = 0; i < quant_left; i++)
    {
        ...
        // if (((long)*f_pos / itemsize) != (((long)*f_pos + bytes_left) / itemsize))
        // {
        ...
        *f_pos += nbytes; /* обновление позиции файла */
        ...
        if (dev->size < *f_pos)
            dev->size = *f_pos;
        //printk(KERN_INFO "write: dptr->data[s_pos] %s\n", dptr->data[s_pos]);
        retval += nbytes;
    }
    //*f_pos += init_bytes_left;
    printk("Доп. запись выполена");
/* Дополнение кода */
out:
    up(&dev->sem);
